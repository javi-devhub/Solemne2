...
describe('VisibilitySystem', () => {
  it('shows item only to player 1', () => {
    const item: Item = { id: 'i1', name: '', description: '', icon: '', sprite: '', isUsable: false, isKeyItem: false, visibleToPlayer: 1 }
    expect(isVisibleToPlayer(item, 1)).toBe(true)
    expect(isVisibleToPlayer(item, 2)).toBe(false)
  })

  it('shows item to both players', () => {
    const item: Item = { id: 'i2', name: '', description: '', icon: '', sprite: '', isUsable: false, isKeyItem: false, visibleToPlayer: 'both' }
